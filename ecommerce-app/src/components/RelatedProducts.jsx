...
const RelatedProducts = ({ id }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true)
      const relatedProducts = await fecthRequest(`products/related/${id}`, 'GET');
      if (relatedProducts?.error) setError(relatedProducts.error);
      else {
        setProducts(relatedProducts?.relatedProducts);
      }
      setLoading(false);
    }
    fetchProducts();
